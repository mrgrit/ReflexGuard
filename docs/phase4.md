...
0.8.2부터 관제 수신 IP를 환경변수로 지정한다. 미설정 기본값은 127.0.0.1이며 현재 VM에는 0.0.0.0을 적용했다. 현재 VM은 LAN Origin과 IP 인증서를 적용했다. 아래 localhost 예시는 초기 로컬 구성에 해당하며, 실제 접속은 [접속 안내](access.md)의 설정된 주소를 따른다.
