# ReflexGuard 서비스 개요서 초안 — 0.7.0

작성 기준: 2026-09-25 구현과 검증 기록. 대회 공식 제출 양식을 받은 뒤 항목·분량을 맞춰야 하며, 아래 내용은 현재 프로젝트에 대한 초안이다. 출품 자격·AI 도구 허용 여부·공식 심사 기준 확인 완료를 뜻하지 않는다.
...
뇌 API 계약은 mock과 실제 MaleCNS LIF가 공유한다. 실제 모델은 v1.0 LPLC2→DNp01 187뉴런 축소 회로를 Thor CUDA에서 실행한다. 연결 수는 원본 데이터, 자극/막전위/조향 정규화는 공학적 가정이다. `mock-rules-v1`은 가상 뉴런 출력이고 `malecns-v1.0-lplc2-gf-lif-v1`은 실제 데이터 기반 축소 모델로 구분한다.
...
Phase 6 실제 GPU 모델·서명 자산과 Phase 7 실제/mock 통합 리허설을 완료했다. [최종 보고서](test_report.md)에 복도 3종 충돌 0회, 단절 정지, 원격 정지·감사 검증과 관찰된 timeout 한계를 기록한다. 다음 버전은 Jev 행동 선택을 계획한다. 별도 Attack Lab과 실제 장비 안전 검증은 이번 완료 범위 밖이다.
