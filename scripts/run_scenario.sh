...
if [[ $# -lt 1 || $# -gt 3 ]]; then
  printf 'Usage: run_scenario.sh corridor_basic|corridor_side|corridor_static|corridor_demo [seconds:1..60] [forward|idle|reverse|left|right]\n' >&2
  exit 2
fi
case "$1" in corridor_basic|corridor_side|corridor_static|corridor_demo) world=$1 ;; *) printf 'Unknown world ID\n' >&2; exit 2 ;; esac
seconds=${2:-10}
drive=${3:-forward}
...
status=0
timeout_seconds=$((seconds * 8 + 30))
timeout --signal=TERM --kill-after=5s "${timeout_seconds}s" xvfb-run -a webots   --batch --mode=fast --no-rendering --minimize --stdout --stderr   "$PWD/webots/worlds/$world.wbt" >"$log_dir/webots.log" 2>&1 || status=$?
